...
class Decryption extends PHPUnit\Framework\TestCase
{
    public function oneSymmetric($pass, $cnt, $path)
    {
        ...
        while ($m2[0] instanceof OpenPGP_CompressedDataPacket) {
            $m2 = $m2[0]->data;
        }
        foreach ($m2 as $p) {
    ...
    public function testDecryptCAST5()
    { // Requires mcrypt or openssl
        $this->oneSymmetric('hello', "PGP\n", 'symmetric-cast5.gpg');
    }
    ...
    {
        $m = new OpenPGP_Message([new OpenPGP_LiteralDataPacket("hello\n")]);
        $key = OpenPGP_Message::parse(file_get_contents(dirname(__FILE__).'/data/helloKey.gpg'));
        $em = OpenPGP_Crypt_Symmetric::encrypt($key, $m);

        foreach ($key as $packet) {
            if (!($packet instanceof OpenPGP_SecretKeyPacket)) {
                continue;
            }
    ...
    public function testDecryptSecretKey()
    {
        ...
        $this->assertSame((bool) $skey, true);
    }
...
class Encryption extends PHPUnit\Framework\TestCase
{
    ...
    {
        $data = new OpenPGP_LiteralDataPacket('This is text.', ['format' => 'u', 'filename' => 'stuff.txt']);
        $encrypted = OpenPGP_Crypt_Symmetric::encrypt('secret', new OpenPGP_Message([$data]), $algorithm);

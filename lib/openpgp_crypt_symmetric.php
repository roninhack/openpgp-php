...
class OpenPGP_Crypt_Symmetric
{
    public static function encrypt($passphrases_and_keys, $message, $symmetric_algorithm = 9)
    {
        ...
        $encrypted = [new OpenPGP_IntegrityProtectedDataPacket($cipher->encrypt($to_encrypt))];
    ...
    public static function encryptSecretKey($pass, $packet, $symmetric_algorithm = 9)
    {
        ...
        if (!$packet->s2k) {
            $packet->s2k = new OpenPGP_S2K(Random::string(8));
        }
    ...
    case null:
      ...
      case 3:
        ...
        } elseif (defined('MCRYPT_CAST_128')) {
    ...
    {
        foreach ($m as $p) {
            if ($p instanceof OpenPGP_EncryptedDataPacket) {
                return $p;
            }
        }
